...
export async function run(){
  console.info(`Starting ${INITIAL_PUBLICATION_SYNC_JOB_OPERATION} at ${new Date().toISOString()}`);
  try {

    let activeJobs = await getJobs(INITIAL_PUBLICATION_SYNC_JOB_OPERATION);
    ...
    console.error(`Error while scheduling job ${INITIAL_PUBLICATION_SYNC_JOB_OPERATION}: ${error}`);
    console.error(error);
