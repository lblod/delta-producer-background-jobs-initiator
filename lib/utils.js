...
export async function updateStatusJob(jobUri, status) {
...
export async function getActiveJobs(jobOperation){
  let activeJobs = await getJobs(jobOperation, [ STATUS_BUSY, STATUS_SCHEDULED ]);
  if (activeJobs.length && JOB_TIMEOUT_MINUTES) {
    ...
    for (const job of activeJobs) {
      ...
      if(diff >= timeoutMinutes) {
          await updateStatusJob(job.jobUri, STATUS_FAILED);
      }else{
